...
from clouds.models.modules import dense
from clouds.models.modules.spectral_norm import spectral_norm_fc
...
class DensityAttentionNetwork(nn.Module):
    def __init__(
        self, encoder: Encoder, density: nn.Module,
    ):
    ...
    def forward(
        ...
        outputs = self.encoder(inputs=inputs, position=position, mask=inputs_mask)
        outputs = outputs.reshape(-1, outputs.shape[-1])[inputs_mask.reshape(-1)]
        return self.density(outputs)
...
class TarAttentionNetwork(nn.Module):
    ...
    def forward(
        ...
        outputs = self.encoder(inputs=inputs, position=position, mask=inputs_mask)
        inputs_mask = inputs_mask.reshape(-1)
        outputs = outputs.reshape(-1, outputs.shape[-1])[inputs_mask]
        treatments = treatments.reshape(-1, treatments.shape[-1])[inputs_mask]
        outputs = (1 - treatments) * self.hypothesis_0(
            outputs
        ) + treatments * self.hypothesis_1(outputs)
        return self.density([outputs, treatments,])
